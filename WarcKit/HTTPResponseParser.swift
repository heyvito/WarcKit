...
struct HTTPResponseParser {
    ...
    mutating func parse() throws -> HTTPResponse {
        try data.withUnsafeBytes { ptr in
            ...
            return HTTPResponse(httpVersion: version, statusCode: status, reasonPhrase: reason, headers: headers, bodyData: Data(bodySlice))
        }
    }
    
    mutating func parseHTTPVersionAndStatus(_ ptr: UnsafeRawBufferPointer) throws -> (httpVersion: String, statusCode: Int, reasonPhrase: String) {
        var mark = cursor
        ...
        guard let httpVersion = String(data: versionBytes, encoding: .ascii) else {
            throw WarcKitError.corruptHTTPResponse("Could not read HTTP version")
        }
        advance()
        ...
        guard let rawHttpStatus = String(data: statusBytes, encoding: .ascii) else {
    ...
    mutating func parseHTTPHeader(_ ptr: UnsafeRawBufferPointer) throws -> (String, String) {
        var mark = cursor
        try advanceUntil(":")
        let key = String(data: Data(ptr[mark..<cursor]), encoding: .utf8)!
        guard match(":") else {
            throw WarcKitError.corruptHTTPResponse("Expected a colon after header key")
        }
        mark = cursor
        advanceToCRLF()
    ...
    mutating func parseHTTPHeaders(_ ptr: UnsafeRawBufferPointer) throws -> [(String, String)] {
...
struct HTTPResponse {
    ...
    func headerByName(_ name: String) -> String? {
        return headers.first { $0.0 == name }?.1
    }
    
    func headersByName(_ name: String) -> [String] {
        return headers.filter { $0.0 == name }.map(\.1)
    }
    
    func isGzipEncoded() -> Bool {
        return headersByName("Content-Encoding").contains("gzip")
    }
    
    func decompress() async throws -> Data {
        let decompressor = GzipDecompressor()
        return try await decompressor.unzip(data: bodyData)
    }
}
